...
const RACES = [
  ...
  {id:'r1', name:'Vic HPR Round 2 — Casey Fields', date:'2026-05-02', location:'Casey Fields, Cranbourne East, VIC', distance:80, type:'endurance', notes:'10am–4pm. 6-hour endurance race. Round 2 of the 2026 Victorian HPR Grand Prix Series.'},
  {id:'r2', name:'Vic HPR Round 3 — Sandown Raceway', date:'2026-07-25', location:'Sandown Raceway, Springvale, VIC', distance:100, type:'endurance', notes:'9am–4pm. 7-hour endurance race. Round 3 of the 2026 Victorian HPR Grand Prix Series.'},
  {id:'r3', name:'Vic HPR Round 4 — Casey Fields', date:'2026-10-17', location:'Casey Fields, Cranbourne East, VIC', distance:120, type:'endurance', notes:'9am–5pm. 8-hour endurance race. Series finale — Round 4 of the 2026 Victorian HPR Grand Prix Series.'},
  {id:'r4', name:'Energy Breakthrough — Maryborough 24hr', date:'2026-11-18', location:'Maryborough, VIC', distance:900, type:'multi_day', notes:'18–22 November. The flagship 24-hour HPR endurance race on the 1.58km Maryborough street circuit. Teams of 8 riders.'},
];
...
// ─── Native bridge (iOS WebView ↔ Watch) ──────────────────────────────────
// When the web app is running inside the iOS WKWebView, this bridge lets us
// push a state snapshot to the paired Watch (via the iPhone's
// WatchConnectivity) and receive race-day laps back from the Watch.
// Outside iOS — running in a normal browser — these calls are no-ops.
const _hasNativeBridge = () => !!(window.webkit?.messageHandlers?.tpNative);
function postNative(type, body) {
...
function pushWatchState() {
  ...
  try {
    ...
    const racePhase = phase ? {
      phase: phase.phase,
      label: phase.label,
      description: phase.description,
      raceShortName: (phase.race?.name || '').replace(/^Vic HPR /, '').replace(/ — /, ' · '),
      daysOut: phase.daysOut,
    } : null;
    ...
    if (plan) {
      const sameDayCounts = {};
      todayWorkouts = plan.workouts
        .map((w, i) => ({ w, i }))
        .filter(({ w }) => dayMap[w.day] === todayDay)
        .map(({ w }, _i) => {
          const key = `${w.week}-${w.day}`;
          const idx = sameDayCounts[key] || 0;
          sameDayCounts[key] = idx + 1;
          const checkKey = `${planId}-${w.week}-${w.day}-${idx}`;
          return {
            planId,
            week: w.week,
            day: w.day,
            ...
            intensity: (w.intensity || 'moderate').toLowerCase(),
            completed: !!(userChecklist && userChecklist[checkKey]),
          };
        });
    }
    const completedWorkouts = (userWorkouts || []).slice(0, 8).map(w => {
      const d = w.date ? (w.date.toDate ? w.date.toDate() : new Date(w.date)) : new Date();
      return {
        id: w._id || (w.name + '-' + d.getTime()),
        name: w.name || 'Workout',
        durationMinutes: w.duration || 0,
        date: d.getTime() / 1000,
        avgHeartRate: w.heartRate || w.heartRateAvg || null,
...
// createdAt }. Generated by hitting /.netlify/functions/ai-coach with a
// widget-flavoured prompt that includes the athlete's recent state, then
...
const AI_WIDGET_KINDS = {
  ...
  metric:   { fg: 'var(--success)', bg: 'rgba(var(--success-rgb),.08)',   border: 'rgba(var(--success-rgb),.25)',   icon: '📊' },
  warning:  { fg: 'var(--warning)', bg: 'rgba(var(--warning-rgb),.08)',  border: 'rgba(var(--warning-rgb),.25)',  icon: '⚠️' },
...
function renderAiWidgets() {
  ...
  let html = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
    ...
    <button id="ai-widget-generate" style="display:flex;align-items:center;gap:4px;font-size:11px;font-weight:700;background:rgba(var(--primary-rgb),.12);border:1px solid rgba(var(--primary-rgb),.3);color:var(--primary);padding:4px 10px;border-radius:14px;cursor:pointer"><svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 2v4"/><path d="M12 18v4"/><path d="M4.93 4.93l2.83 2.83"/><path d="M16.24 16.24l2.83 2.83"/><path d="M2 12h4"/><path d="M18 12h4"/><path d="M4.93 19.07l2.83-2.83"/><path d="M16.24 7.76l2.83-2.83"/></svg><span>Ask coach</span></button>
  </div>`;
  if (widgets.length === 0) {
  ...
  widgets.forEach(w => {
    const k = AI_WIDGET_KINDS[w.kind] || AI_WIDGET_KINDS.tip;
    html += `<div class="ai-widget" data-aiw-id="${escHtml(w.id)}" style="background:${k.bg};border:1px solid ${k.border};border-radius:12px;padding:10px 12px;margin-bottom:8px">
      <div style="display:flex;align-items:flex-start;gap:8px">
        <div style="font-size:14px;line-height:1.2">${k.icon}</div>
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:2px">
            <div style="font-size:12px;font-weight:800;color:${k.fg};letter-spacing:.02em">${escHtml(w.title || 'Coach insight')}</div>
            <button class="ai-widget-dismiss" data-aiw-id="${escHtml(w.id)}" style="background:none;border:none;color:var(--muted-fg);font-size:14px;line-height:1;padding:0;cursor:pointer">×</button>
          </div>
          <div style="font-size:12.5px;color:var(--fg);line-height:1.45">${escHtml(w.body || '')}</div>
        </div>
      </div>
    </div>`;
  });
...
async function generateAiWidget(extraContext) {
  ...
  try {
    ...
    const phase = computeRacePhase();
    const recent = (userWorkouts || []).slice(0, 6).map(w => {
      const d = w.date ? (w.date.toDate ? w.date.toDate() : new Date(w.date)) : null;
      return { name: w.name, type: w.type, duration: w.duration, dateAgo: d ? Math.round((Date.now() - d) / 86400000) + 'd ago' : '?' };
    });
    const planId = userProfile?.activePlanId || null;
    const yearTier = (userProfile?.yearLevel || '?') + '/' + (userProfile?.fitnessLevel || '?');
    const skipLog = JSON.parse(localStorage.getItem('tp_skip_log') || '[]').slice(0, 5);
    const promptContext = {
      year_tier: yearTier,
      active_plan: planId,
    ...
    const m = reply.match(/<<WIDGET>>([\s\S]*?)<<END>>/);
    if (!m) throw new Error('Unexpected reply format');
    const parsed = JSON.parse(m[1].trim());
    const widget = {
      id: Date.now().toString(36) + Math.random().toString(36).slice(2, 7),
      kind: AI_WIDGET_KINDS[parsed.kind] ? parsed.kind : 'tip',
      title: String(parsed.title || 'Coach insight').slice(0, 60),
      body: String(parsed.body || '').slice(0, 240),
      createdAt: new Date().toISOString(),
    };
    const list = loadAiWidgets();
    list.unshift(widget);
    saveAiWidgets(list);
    if (currentPage === 'today') renderToday();
    if (typeof showToast === 'function') showToast('Coach insight added.', 'success');
  } catch(e) {
    if (typeof showToast === 'function') showToast('Coach is busy — try again shortly.', 'warn');
    if (btn) {
      btn.disabled = false; btn.style.opacity = '1';
      const span = btn.querySelector('span'); if (span) span.textContent = 'Ask coach';
    }
  }
}
// App State
...
const APP_VERSION = '20260430-r20';
const CHANGELOG = [
...
function openErrorDiagnostics(entry, showLog) {
  ...
  ov.querySelector('#diag-ai-btn')?.addEventListener('click', async () => {
    ...
    try {
      const diagPrompt = `A user got this error in a school HPR training app. Give a SHORT (3-4 sentences max) friendly explanation of what went wrong and exactly how to fix it. ${roleContext}
...
function extractAllExercises() {
  ...
  const FLOOR_EXERCISES = [
    ...
    { name: 'Glute Bridge', type: 'Glutes', videoUrl: 'https://www.youtube.com/watch?v=WtilA9IJX1c', desc: 'Lie on your back with knees bent and feet flat on the floor hip-width apart. Push your hips up toward the ceiling until your body forms a straight line from shoulders to knees. Squeeze your glutes hard at the top and hold for two to four seconds before lowering slowly. Your glutes are the single most powerful muscle for HPR pedalling — this exercise specifically activates and strengthens them through the exact hip extension pattern used in every pedal stroke.' },
...
function renderGoals() {
  ...
  userGoals.forEach((g, idx) => {
    ...
    if (g.type === 'workouts') current = userWorkouts.length;
      ...
      current = thisCnt;
      // Make the target explicit. Was just "(last week: 3)" — students didn't
      // realise the goal was actually 4 (last week + 1).
      unitLabel = `workouts · target ${displayTarget} (last week ${lastCnt})`;
    }
    ...
    html += '<circle cx="28" cy="28" r="' + radius + '" fill="none" stroke="' + (done ? "var(--success)" : "var(--primary)") + '" stroke-width="4" stroke-dasharray="' + circ + '" stroke-dashoffset="' + offset + '" stroke-linecap="round"/>';
...
function renderToday() {
  ...
  // dismiss button; the section header has a "✨ Ask coach" trigger.
  html += renderAiWidgets();
  ...
  if (racePhase) {
    ...
    const phaseColors = {
      'base':      { bg: 'rgba(var(--success-rgb),.10)',  border: 'rgba(var(--success-rgb),.25)',  fg: 'var(--success)' },
      'build':     { bg: 'rgba(var(--warning-rgb),.10)', border: 'rgba(var(--warning-rgb),.25)', fg: 'var(--warning)' },
      'peak':      { bg: 'rgba(var(--destructive-rgb),.10)',  border: 'rgba(var(--destructive-rgb),.25)',  fg: 'var(--destructive)' },
    ...
    const raceShort = race.name.replace(/^Vic HPR /, '').replace(/ — /, ' · ');
    let gearHtml = '';
    if (phase === 'race-week') {
  ...
  // Bind AI widget controls
  $('ai-widget-generate')?.addEventListener('click', () => {
    haptic('light');
    generateAiWidget();
  });
  document.querySelectorAll('.ai-widget-dismiss').forEach(btn => {
    btn.addEventListener('click', () => {
      haptic('light');
...
function openExerciseTracker(key, name, desc, duration, exercisesJson) {
  ...
  function renderTracker() {
    ...
    let html = `<div style="display:flex;align-items:center;padding:12px 16px;padding-top:calc(12px + var(--safe-t));border-bottom:1px solid var(--border);flex-shrink:0">
      ...
      <button id="et-live-btn" style="font-size:11px;font-weight:700;padding:6px 12px;border-radius:8px;border:1.5px solid var(--primary);background:rgba(var(--primary-rgb),.1);color:var(--primary);cursor:pointer">\u25b6 Live Mode</button>
    </div>
...
function completionToastMsg(activePlan, key) {
  ...
  const raceShort = phase.race.name.replace(/^Vic HPR /, '').replace(/ — /, ' · ');
...
async function skipTodaysWorkouts() {
  ...
  const raceShort = phase ? phase.race.name.replace(/^Vic HPR /, '').replace(/ — /, ' · ') : null;
...
function renderWorkouts() {
  ...
  if (userWorkouts.length === 0) {
    ...
    userWorkouts.forEach((w, idx) => {
      ...
      const typeLabels = {hpv:'HPR',ride:'Ride',run:'Run',walk:'Walk',treadmill:'Treadmill',gym:'Gym',cycle:'Cycle'};
  ...
  try {
    ...
    for (const { day, stints } of stintFetches) {
      ...
      if (myStint) {
        ...
        html += `<div style="background:rgba(var(--primary-rgb),.06);border:1px solid rgba(var(--primary-rgb),.15);border-radius:8px;padding:10px 12px;margin-bottom:10px">
...
function openCreateTeamSheet() {
  ...
  $('sheet-content').innerHTML = `
    ...
    <div class="form-group">
      ...
      <input class="input" type="text" id="team-req-name" placeholder="e.g. Caulfield Grammar HPR" maxlength="60" value="${escHtml(userProfile?.clubName || '')}">
    </div>
    <div class="form-group">
      <label class="label" for="team-req-desc">Short Description</label>
      <input class="input" type="text" id="team-req-desc" placeholder="e.g. Year 8-12 HPR racing program" maxlength="120">
    </div>
...
async function promoteToCoach(uid, btn) {
  ...
  try {
    ...
    btn.replaceWith(Object.assign(document.createElement('span'), {
      style: 'font-size:11px;font-weight:700;color:var(--primary);padding:6px 10px;border:1px solid rgba(var(--primary-rgb),.4);border-radius:999px',
      textContent: 'COACH',
    }));
  } catch (e) {
...
function renderGodAdminPanel() {
  ...
  return `
    <div style="margin-top:20px;padding:16px;background:rgba(var(--primary-rgb),.06);border:1px solid rgba(var(--primary-rgb),.2);border-radius:14px">
